...
Transform your Android smartphone into a fully functional PC steering wheel with **Force Feedback (FFB)** support, ideal for racing games and simulators. 
...
* **Force Feedback (FFB):** Feel the game's physics on your phone through haptic vibration.
...
2. Download `VWheel_Server_v1.0.2_Installer.exe`.
...
5. On the first launch, it will prompt you to configure vJoy. Set buttons to 32, check "Enable Effects", and hit Apply.

### 2. Prepare the Phone (Client)
1. From the [Releases](../../releases) section, download `VWheel_Client_v1.0.2.apk`.
...
* **Solution 2:** Make sure you don't have `vJoyConf` or `vJoyMonitor` open in the background, as they block access to the steering wheel.
* **Solution 3:** Open the "Configure vJoy" program on your PC, ensure the **Enable Effects** (FFB) box is checked, and click Apply.

**❌ Phone won't connect to the PC server**
* Verify your Wi-Fi network is set to "Private" in Windows.
* Check that the Windows Firewall isn't blocking VWheel or UDP connections.
...
* **Force Feedback (FFB):** Siente las físicas del juego en tu teléfono mediante vibración háptica.
...
2. Descarga el archivo `VWheel_Server_v1.0.2_Installer.exe`.
...
5. En el primer inicio, te pedirá configurar vJoy. Pon los botones en 32, marca "Enable Effects" (FFB) y haz clic en Apply.

### 2. Preparar el Celular (Cliente)
1. En la misma sección de [Releases](../../releases), descarga el archivo `VWheel_Client_v1.0.2.apk`.
